...
void AliAnalysisTaskTIdentityPID::Initialize()
{
  ...
  fESDtrackCuts->SetMaxDCAToVertexZ(10);
  //
  // ------------------------------------------------
  //
  // ------------------------------------------------
...
void AliAnalysisTaskTIdentityPID::UserCreateOutputObjects()
{
  ...
  {
    const Int_t ndim=5;
    Int_t nbins0[ndim]  ={4,8, 40      ,16       ,40  };
    Double_t xmin0[ndim]={0,0, 0.2     ,-0.8     ,0.  };
    Double_t xmax0[ndim]={4,80,3.2     , 0.8     ,6.25};
...
//________________________________________________________________________
void AliAnalysisTaskTIdentityPID::CalculateFastGenVsFullMCHigherMoments()
{
...
void AliAnalysisTaskTIdentityPID::GetExpecteds(AliESDtrack *track, Float_t closestPar[3])
{
  ...
  if (fEventInfo){
    ...
    fPiNSigmasTOF = fPIDResponse->NumberOfSigmasTOF(track, AliPID::kPion,     fPIDResponse->GetTOFResponse().GetTimeZero());
    fKaNSigmasTOF = fPIDResponse->NumberOfSigmasTOF(track, AliPID::kKaon,     fPIDResponse->GetTOFResponse().GetTimeZero());
    fPrNSigmasTOF = fPIDResponse->NumberOfSigmasTOF(track, AliPID::kProton,   fPIDResponse->GetTOFResponse().GetTimeZero());
    fDeNSigmasTOF = fPIDResponse->NumberOfSigmasTOF(track, AliPID::kDeuteron, fPIDResponse->GetTOFResponse().GetTimeZero());
  }
  //
...
//________________________________________________________________________
Bool_t AliAnalysisTaskTIdentityPID::CheckIfFromAnyResonance(AliMCParticle *trackMCgen)
{
...
void AliAnalysisTaskTIdentityPID::SelectCleanSamplesFromV0s(AliESDv0 *v0, AliESDtrack *track0, AliESDtrack *track1)
{
  ...
  const Double_t massProton  =TDatabasePDG::Instance()->GetParticle(kPDGpr)->Mass();
  const Double_t massPion    =TDatabasePDG::Instance()->GetParticle(kPDGpi)->Mass();
...
void AliAnalysisTaskTIdentityPID::DumpDownScaledTree()
{
  ...
  for (Int_t itrack=0;itrack<nNumberOfTracks;++itrack)
  {
    ...
    if ( (r.Rndm()*(qP*qP) < 0.05) || itsHighDeDx )
    {
      ...
      "exppr="                << fDEdxPr               <<
      "expde="                << fDEdxDe               <<
      "tofnsigmael="          << fElNSigmasTOF         <<
      "tofnsigmapi="          << fPiNSigmasTOF         <<
      "tofnsigmaka="          << fKaNSigmasTOF         <<
      "tofnsigmapr="          << fPrNSigmasTOF         <<
      "tofnsigmade="          << fDeNSigmasTOF         <<
      //
      "flag="                 << flag                  <<
