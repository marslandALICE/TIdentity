...
// Initial Settings
Bool_t automaticKS            = kFALSE;     // find kurtosis and skewness automatically form the MC samples 
Bool_t KSestimator            = kFALSE;
Bool_t Systematic             = kTRUE;     // for the systematics study jump directly to the 6th iter
Bool_t dump                   = kFALSE;
Bool_t normalisedCleanSamples = kFALSE;
Bool_t pp                     = kFALSE;
Bool_t MC                     = kTRUE;      // used for the pure known MC particle spectrum
Bool_t centIntegrated         = kTRUE;      // true if centintegrated samples are used for par estimation
Int_t setKsFreeDom            = 1;          // if 0 fixed ks is used, if 1 flexibele ks is used.
...
Int_t freedomForPion   = 3;        // up to freedomForPion iteration fix all particle Kurtosis and skewness then let pion KSfree
...
TGraphErrors *grprCleanMean=0x0, *grprMeanExScaled=0x0, *grprSigmaExScaled=0x0, *grprCleanSigma=0x0, *grprCleanAmpScaledSmooth=0x0, *grprCleanSigmaScaledSmooth=0x0, *grprFitAmpOutlierSmooth=0x0;
  ...
  // For h2DMC analysis
TGraphErrors *grelMCSigmaScaled=0x0, *grpiMCSigmaScaled=0x0, *grkaMCSigmaScaled=0x0, *grprMCSigmaScaled=0x0; 
TGraphErrors *grelMCMeanScaled=0x0,  *grpiMCMeanScaled=0x0,  *grkaMCMeanScaled=0x0,  *grprMCMeanScaled=0x0;  
TGraphErrors *grelMCAmpScaled=0x0,   *grpiMCAmpScaled=0x0,   *grkaMCAmpScaled=0x0,   *grprMCAmpScaled=0x0;              
...
void          SmoothAmplitudes(TString fileFit, TString fileSample, TString fileOut, Int_t iIter);
...
void          FitMCSamplesForTIDEN(TH2D **hMCSamplesCInt,Int_t nMomSlices, Double_t pMin, Double_t pMax);
void          EstimateKS(Int_t iks, TH2D *h2DKS, TH2D **hCleanSamples, TH2D **hMCSamplesCInt, TH2D **hExpectedMean, TH2D **hExpectedSigma);
void          FitAllSamples(const Double_t ptMin, const Double_t ptMax,TH2D *h2D, TH2D **hCleanSamples, TH2D **hMCSamplesCInt, TH2D **hExpectedMean, TH2D **hExpectedSigma, TString sampleFileName);
void          IterativeFitting(Int_t iIter, const Double_t ptMin, const Double_t ptMax, TH2D *h2D, TString fileIn, TString readSmooth, TString readSamp);
void          ProduceGraphs(TTree *t, TObjArray *arr, TObjArray *barr, TGraphErrors **grCleanTmp, TGraph **grCleanSmoothTmp);
void          ApplyOutlierSmooth(TTree *t, TObjArray *arr, TObjArray *barr, TGraphErrors **grTmp, TGraph **grSmoothTmp);
...
TMatrixD     *FitParticleSample(Int_t ksFreeDom,TH1D *hClean, Double_t *parSample, Double_t *arrMean, Double_t *arrSigma, ParticleType pSpecy, Int_t sampleType);
...
TGraphErrors *ApplyScalingWrtExpected(TString exp, TString fit, const Double_t safeP, TObjArray * arr, TTree *t);
TGraphErrors *ApplyScalingWrtSample(TString sample, TString fit, const Double_t safeP, TObjArray * arr, TObjArray * arrSample, TTree *tSample, TGraph **grScaledSmooth,Int_t k);
TGraphErrors *ProduceWindowGraphs(Int_t index, TString str, TTree *tree);
...
Int_t         CalculateNBinsP(Int_t ptMin, Int_t ptMax);


// Set functions for the parameters
void          SetTotalFitParameters(Int_t iIter, Int_t islice,TF1* total, Double_t *arrMean, Double_t *arrSigma, Double_t *arrMeanWindow, Double_t *cleanParMean, Double_t *MCPars, TH1D* h1D, Double_t pt);
...
// }
// -------------------------------------------------------------------------------------------------------
void PIDIterativeFittingMC(TString fileData, Int_t nMomSlices, Double_t ptMin, Double_t ptMax, Int_t maxIter=5){
    
  //
  // Apply gaussian fits to the ptot slices --> Main Function
  /*
  
  cd /hera/alice/marsland/pFluct/files/analysis/rootFilesPbPb/fullStat/ForAnar/PbPb_MC/test
  aliroot -l -b 
  .L ~/PHD/macros/marsland_EbyeRatios/PIDIterativeFittingMC.C+
  TString fileMC ="/hera/alice/marsland/pFluct/files/analysis/Data/PbPb/MC/MC_PbPb_Reference/Hists/centIntegrated/Hists_PbPb_eta_0.0_0.2_cent_0_5.root"
  PIDIterativeFittingMC(fileMC,10,0.5,0.6,0);  2> err.log
  */
  //
...
//   TString objFileName = (TString)((objArr1->At((Int_t)objArr1->GetLast()))->GetName());
  ...
  gSystem->Exec("cp /u/marsland/PHD/macros/marsland_EbyeRatios/PIDIterativeFittingMC.C .");
  ...
  TH2D ** hMCSamplesCInt  = new TH2D *[6];
  ...
  cout << " ================================== "    << endl;
  cout << " Hist name        = " << h2D->GetName()  << endl;
  ...
  hMCSamplesCInt[0] = (centIntegrated)? (TH2D *)f->Get("h2MCelectronCInt") : (TH2D *)hMCSamples[0]->Clone("h2MCelectronCInt");
  hMCSamplesCInt[1] = (centIntegrated)? (TH2D *)f->Get("h2MCpionCInt")     : (TH2D *)hMCSamples[1]->Clone("h2MCpionCInt");
  hMCSamplesCInt[2] = (centIntegrated)? (TH2D *)f->Get("h2MCkaonCInt")     : (TH2D *)hMCSamples[2]->Clone("h2MCkaonCInt");
  hMCSamplesCInt[3] = (centIntegrated)? (TH2D *)f->Get("h2MCprotonCInt")   : (TH2D *)hMCSamples[3]->Clone("h2MCprotonCInt");
  ...
  hMCSamplesCInt[0] -> Write("h2MCelectronCInt");
  hMCSamplesCInt[1] -> Write("h2MCpionCInt");
  hMCSamplesCInt[2] -> Write("h2MCkaonCInt");
  hMCSamplesCInt[3] -> Write("h2MCprotonCInt");
  ...
  cout << " =========== Sumw2 ============ " << endl;
  h2D               -> Sumw2();
  hMCSamplesCInt[0] -> Sumw2();
  hMCSamplesCInt[1] -> Sumw2();
  hMCSamplesCInt[2] -> Sumw2();
  hMCSamplesCInt[3] -> Sumw2();
  hMCSamples[0]     -> Sumw2();
  hMCSamples[1]     -> Sumw2();
  hMCSamples[2]     -> Sumw2();
  hMCSamples[3]     -> Sumw2();
  ...
  if (KSestimator){
    for (Int_t iks = 0; iks<2; iks++){      
      ...
      EstimateKS(iks,h2DKS,hCleanSamples,hMCSamplesCInt,hExpectedMean,hExpectedSigma);
    }
  }
  delete h2DKS;
  delete debugFile;
  ...
  FitAllSamples(ptMin,ptMax,h2D,hCleanSamples,hMCSamplesCInt,hExpectedMean,hExpectedSigma,fileSample);
  FitMCSamplesForTIDEN(hMCSamples,nMomSlices,ptMin,ptMax);

  // delete unnecessary histograms in memory 
  for (Int_t icache=0; icache<6; icache++)
  {
    delete hMCSamplesCInt[icache];
  ...
  for (Int_t iIter = 0; iIter<10; iIter++){
    ...
    IterativeFitting(iIter,ptMin,ptMax,h2D,fileFit,readSmooth,fileSample);
    
    cout << " =========== Iteration = " << iIter << "   has finished go to iteration = " << iIter+1 << endl;
    if (nSlice>=40) SmoothAmplitudes(fileFit,fileSample,fileOut,iIter);
  }
...
//   Produce smooth and normal graphs
  ProduceGraphs(treeFit  , GrArrFit  , branchArrFit  , grFit  , grFitSmooth);  
  ProduceGraphs(treeClean, GrArrClean, branchArrClean, grClean, grCleanSmooth); 
  ProduceGraphs(treeMC   , GrArrMC   , branchArrMC   , grMC   , grMCSmooth);    
  
//   Apply outlier removal to fits
  if (!MC) ApplyOutlierSmooth(treeFit, GrArrOutlierSmooth, branchArrFit, grOutlierCut, grOutlierSmooth);   
  
//   Apply Scalings for expected mean and sigma
  grScaledEx[0] = ApplyScalingWrtExpected("elMean","elFitMean",0.32,GrArrFit,treeFit);
  grScaledEx[1] = ApplyScalingWrtExpected("piMean","piFitMean",0.6,GrArrFit,treeFit);
  grScaledEx[2] = ApplyScalingWrtExpected("kaMean","kaFitMean",0.4,GrArrFit,treeFit);
  grScaledEx[3] = ApplyScalingWrtExpected("prMean","prFitMean",0.6,GrArrFit,treeFit);
  grScaledEx[4] = ApplyScalingWrtExpected("elSigma","elFitSigma",0.4,GrArrFit,treeFit);
  grScaledEx[5] = ApplyScalingWrtExpected("piSigma","piFitSigma",0.6,GrArrFit,treeFit);
  grScaledEx[6] = ApplyScalingWrtExpected("kaSigma","kaFitSigma",0.4,GrArrFit,treeFit);
  grScaledEx[7] = ApplyScalingWrtExpected("prSigma","prFitSigma",0.6,GrArrFit,treeFit);
  for (Int_t i=0; i<8; i++) GrArrScaledEx->AddAt(grScaledEx[i],i);

//   Apply Scalings for MC data mean and sigma
  grScaledMC[0]  = ApplyScalingWrtSample("elMCAmp","elFitAmp",0.32,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,0);
  grScaledMC[1]  = ApplyScalingWrtSample("piMCAmp","piFitAmp",0.6,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,1);
  grScaledMC[2]  = ApplyScalingWrtSample("kaMCAmp","kaFitAmp",0.7,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,2);
  grScaledMC[3]  = ApplyScalingWrtSample("prMCAmp","prFitAmp",1.25,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,3);
  grScaledMC[4]  = ApplyScalingWrtSample("elMCMean","elFitMean",0.32,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,4);
  grScaledMC[5]  = ApplyScalingWrtSample("piMCMean","piFitMean",0.55,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,5);
  grScaledMC[6]  = ApplyScalingWrtSample("kaMCMean","kaFitMean",0.32,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,6);
  grScaledMC[7]  = ApplyScalingWrtSample("prMCMean","prFitMean",0.6,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,7);
  grScaledMC[8]  = ApplyScalingWrtSample("elMCSigma","elFitSigma",0.32,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,8);
  grScaledMC[9]  = ApplyScalingWrtSample("piMCSigma","piFitSigma",0.55,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,9);
  grScaledMC[10] = ApplyScalingWrtSample("kaMCSigma","kaFitSigma",0.32,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,10);
  grScaledMC[11] = ApplyScalingWrtSample("prMCSigma","prFitSigma",0.6,GrArrFit,GrArrMC,treeMC,grScaledMCSmooth,11);
...
//   Apply Scalings for MC data mean and sigma KAON and PROTON AMPLITUDE !!!!!!
  grScaledClean[0]  = ApplyScalingWrtSample("elCleanAmp","elFitAmp"  ,0.4,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,0);
  grScaledClean[1]  = ApplyScalingWrtSample("piCleanAmp","piFitAmp"  ,0.6,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,1);
  grScaledClean[2]  = ApplyScalingWrtSample("kaCleanAmp","kaFitAmp"  ,0.65,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,2);
  grScaledClean[3]  = ApplyScalingWrtSample("prCleanAmp","prFitAmp"  ,1.25,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,3);
  
  grScaledClean[4]  = ApplyScalingWrtSample("elCleanMean","elFitMean",0.4,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,4);
  grScaledClean[5]  = ApplyScalingWrtSample("piCleanMean","piFitMean",0.6,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,5);
  grScaledClean[6]  = ApplyScalingWrtSample("kaCleanMean","kaFitMean",0.65,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,6);
  grScaledClean[7]  = ApplyScalingWrtSample("prCleanMean","prFitMean",0.8,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,7);
  
  grScaledClean[8]  = ApplyScalingWrtSample("elCleanSigma","elFitSigma",0.4,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,8);
  grScaledClean[9]  = ApplyScalingWrtSample("piCleanSigma","piFitSigma",0.6,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,9);
  grScaledClean[10] = ApplyScalingWrtSample("kaCleanSigma","kaFitSigma",0.65,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,10);
  grScaledClean[11] = ApplyScalingWrtSample("prCleanSigma","prFitSigma",0.75,GrArrFit,GrArrClean,treeClean,grScaledCleanSmooth,11);
...
//   Make windows around mean of particles 
  grWindowsAroundMean[0]  = ProduceWindowGraphs(1  ,"(elMean-elWindow):p",treeFit);
  grWindowsAroundMean[1]  = ProduceWindowGraphs(2  ,"(elMean+elWindow):p",treeFit);
  grWindowsAroundMean[2]  = ProduceWindowGraphs(3  ,"elMean:p"           ,treeFit);
  grWindowsAroundMean[3]  = ProduceWindowGraphs(4  ,"(piMean-piWindow):p",treeFit);
  grWindowsAroundMean[4]  = ProduceWindowGraphs(5  ,"(piMean+piWindow):p",treeFit);
  grWindowsAroundMean[5]  = ProduceWindowGraphs(6  ,"piMean:p"           ,treeFit);
  grWindowsAroundMean[6]  = ProduceWindowGraphs(7  ,"(kaMean-kaWindow):p",treeFit);
  grWindowsAroundMean[7]  = ProduceWindowGraphs(8  ,"(kaMean+kaWindow):p",treeFit);
  grWindowsAroundMean[8]  = ProduceWindowGraphs(9  ,"kaMean:p"           ,treeFit);
  grWindowsAroundMean[9]  = ProduceWindowGraphs(10 ,"(prMean-prWindow):p",treeFit);
  grWindowsAroundMean[10] = ProduceWindowGraphs(11 ,"(prMean+prWindow):p",treeFit);
  grWindowsAroundMean[11] = ProduceWindowGraphs(12 ,"prMean:p"           ,treeFit);
...
void EstimateKS(Int_t iks, TH2D *h2DKS, TH2D **hCleanSamples, TH2D **hMCSamplesCInt, TH2D **hExpectedMean, TH2D **hExpectedSigma){
  ...
  for (Int_t islice = 0; islice<nbins; islice++){
    ...
    h1MCSamples[0]    = GetClean1DSlice(hMCSamplesCInt[0]   ,"h1ElectronMC_slice_%f",ptDownBin,ptUpBin);
    h1MCSamples[1]    = GetClean1DSlice(hMCSamplesCInt[1]   ,"h1PionMC_slice_%f"    ,ptDownBin,ptUpBin);
    h1MCSamples[2]    = GetClean1DSlice(hMCSamplesCInt[2]   ,"h1KaonMC_slice_%f"    ,ptDownBin,ptUpBin);
    h1MCSamples[3]    = GetClean1DSlice(hMCSamplesCInt[3]   ,"h1ProtonMC_slice_%f"  ,ptDownBin,ptUpBin);
    ...
    // Safe region Kurtosis 
      ...
      if (pt>=0.5 && pt<=0.8)   hMCKurtosis[kPion]     .Fill(piParMC[3]);
      if (pt>=0.7 && pt<=0.9)   hMCKurtosis[kKaon]     .Fill(kaParMC[3]);
      if (pt>=0.9 && pt<=1.1)   hMCKurtosis[kProton]   .Fill(prParMC[3]);
    ...
    // Safe region Skewness
      ...
      if (pt>=0.6 && pt<=0.8)   hMCSkewness[kPion]     .Fill(piParMC[4]);
      if (pt>=0.7 && pt<=0.9)   hMCSkewness[kKaon]     .Fill(kaParMC[4]);
      if (pt>=0.9 && pt<=1.1)   hMCSkewness[kProton]   .Fill(prParMC[4]);
...
// -------------------------------------------------------------------------------------------------------
void FitMCSamplesForTIDEN(TH2D **hMCSamples,Int_t nMomSlices, Double_t pMin, Double_t pMax){
  ...
  g.SetMaxIterations(10000);
  gMinuit->SetMaxIterations(10000);
  TString mcFitFileForTiden       = Form("TreeMCForTiden_Iter%d_%d_%3.1f_%3.1f_%2.1f_%d.root",-1,nMomSlices,pMin,pMax,etaBin,centBin);
  TTreeSRedirector *mcFitstream   = new TTreeSRedirector(mcFitFileForTiden,"recreate");
  TObjArray MCSamplesForTiden(1000);  
  MCSamplesForTiden.SetOwner(kTRUE);
  ...
  for (Int_t islice = 0; islice<nSlice; islice++){
    ...
    ptStep = ((pMax-pMin)/(Double_t)nSlice);
    ptDown = pMin+islice*ptStep;
    ptUp   = ptDown+ptStep;
    pt     = (ptDown+ptUp)/2.;
    
    // small array of mc samples for tiden
    TObjArray * MCSamplesForTidenArr = new TObjArray(8);  
    MCSamplesForTidenArr -> SetOwner(kTRUE); 
    MCSamplesForTidenArr->SetName(Form("MCSamplesForTiden_%d_%4.3f",islice,pt));
  
    // find bins to be used in the projection    
    ptDownBin = (hMCSamples[1]->GetXaxis()->FindBin(ptDown));                               
    ptUpBin   = (hMCSamples[1]->GetXaxis()->FindBin(ptUp));  
                                 
    cout << " islice = " << islice << "  ptDown = " << ptDown << "  ptUp = " << ptUp ;
    cout << " ptDownBin = " << ptDownBin << "  ptUpBin = " << ptUpBin << endl;
    ...
    Double_t * kaParMC = new Double_t[10];
    Double_t * prParMC = new Double_t[10];     
    for (Int_t ipar=0; ipar<10; ipar++) { piParMC[ipar]=0; kaParMC[ipar]=0; prParMC[ipar]=0; elParMC[ipar]=0;}
     
    // Get Clean MC Samples 1d slice
    h1MCSamples[0] = GetClean1DSlice(hMCSamples[0] ,"h1ElectronMCtiden_slice_%f",ptDownBin,ptUpBin);
    h1MCSamples[1] = GetClean1DSlice(hMCSamples[1] ,"h1PionMCtiden_slice_%f"    ,ptDownBin,ptUpBin);
    h1MCSamples[2] = GetClean1DSlice(hMCSamples[2] ,"h1KaonMCtiden_slice_%f"    ,ptDownBin,ptUpBin);
    h1MCSamples[3] = GetClean1DSlice(hMCSamples[3] ,"h1ProtonMCtiden_slice_%f"  ,ptDownBin,ptUpBin);
             
    TMatrixD *elMCMatrix = FitParticleSample(setKsFreeDom, h1MCSamples[0] , elParMC , 0, 0, kElectron, 1);
    TMatrixD *piMCMatrix = FitParticleSample(setKsFreeDom, h1MCSamples[1] , piParMC , 0, 0, kPion    , 1);
    TMatrixD *kaMCMatrix = FitParticleSample(setKsFreeDom, h1MCSamples[2] , kaParMC , 0, 0, kKaon    , 1);
    TMatrixD *prMCMatrix = FitParticleSample(setKsFreeDom, h1MCSamples[3] , prParMC , 0, 0, kProton  , 1);
    
    h1MCSamples[0]    -> SetName(Form("MC_ElectronCInt_%d_%4.3f" ,islice,pt));
    h1MCSamples[1]    -> SetName(Form("MC_PionCInt_%d_%4.3f"     ,islice,pt));
    h1MCSamples[2]    -> SetName(Form("MC_KaonCInt_%d_%4.3f"     ,islice,pt));
    h1MCSamples[3]    -> SetName(Form("MC_ProtonCInt_%d_%4.3f"   ,islice,pt));
    
    MCSamplesForTidenArr -> AddAt(h1MCSamples[0],0);
    MCSamplesForTidenArr -> AddAt(h1MCSamples[1],1);
    MCSamplesForTidenArr -> AddAt(h1MCSamples[2],2);
    MCSamplesForTidenArr -> AddAt(h1MCSamples[3],3);   
    MCSamplesForTiden . AddAt(MCSamplesForTidenArr ,islice);
    
    // Fit results dump ttrees
    Int_t iteration = -1;
    Double_t totalChi2    = piParMC[6];  // pi Chi2
    Double_t normNDF      = 0.;
    Double_t totChi2      = piParMC[6];
    Double_t pValue       = 0.;
    Double_t ksValue      = 0.;
    mcFitstream->GetFile()->cd();
    *mcFitstream << "IdMethodInput" <<
        ...
        "elKurtosis="  << elParMC[3]     <<
        "piKurtosis="  << piParMC[3]     <<
        "kaKurtosis="  << kaParMC[3]     <<
        "prKurtosis="  << prParMC[3]     <<
    ...
    *mcFitstream << "IdMethodInputDebug" <<
  ...
  mcFitstream->GetFile()->cd();
  MCSamplesForTiden.Write("MCSamplesForTiden" ,TObject::kSingleKey);
  MCSamplesForTiden.Delete();
...
//     ptUpBin   = (h2D->GetXaxis()->FindBin(ptMin)-1)+2*islice+2;                                // TO FIX
    ...
    ptUpBin   = (h2D->GetXaxis()->FindBin(ptUp));  
    ...
    //  Make the fits        
    TMatrixD *elCleanMatrix = FitParticleSample(0, h1CleanSamples[0] , elParClean, arrMean, arrSigma, kElectron, 0);
    TMatrixD *piCleanMatrix = FitParticleSample(0, h1CleanSamples[1] , piParClean, arrMean, arrSigma, kPion    , 0);
    TMatrixD *kaCleanMatrix = FitParticleSample(0, h1CleanSamples[2] , kaParClean, arrMean, arrSigma, kKaon    , 0);
    TMatrixD *prCleanMatrix = FitParticleSample(0, h1CleanSamples[3] , prParClean, arrMean, arrSigma, kProton  , 0);
    
    TMatrixD *elMCMatrix    = FitParticleSample(0, h1MCSamples[0]    , elParMC   , arrMean, arrSigma, kElectron, 1);
    TMatrixD *piMCMatrix    = FitParticleSample(0, h1MCSamples[1]    , piParMC   , arrMean, arrSigma, kPion    , 1);
    TMatrixD *kaMCMatrix    = FitParticleSample(0, h1MCSamples[2]    , kaParMC   , arrMean, arrSigma, kKaon    , 1);
    TMatrixD *prMCMatrix    = FitParticleSample(0, h1MCSamples[3]    , prParMC   , arrMean, arrSigma, kProton  , 1);
...
// -------------------------------------------------------------------------------------------------------
void IterativeFitting(Int_t iIter, const Double_t ptMin, const Double_t ptMax, TH2D *h2D, TString fileFit, TString readSmooth, TString readSamp){
  ...
  Int_t allBins = CalculateNBinsP(ptMin,ptMax);
  ...
  for (Int_t islice = 0; islice<nSlice; islice++){
    ...
    SetTotalFitParameters(iIter,islice,total,arrMean,arrSigma,arrMeanWindow,cleanParMean,MCPars,h1D,pt); 
...
void SetTotalFitParameters(Int_t iIter, Int_t islice, TF1* total, Double_t *arrMean, Double_t *arrSigma, Double_t *arrMeanWindow, Double_t *cleanParMean, Double_t * MCPars, TH1D* h1D, Double_t pt){
  ...
  arrMeanWindow[3] = GetClosestParticleMean(arrSigma[3],arrMean[3],arrMean[1],arrMean[2],arrMean[0]);
    ...
    //   set 1 iter params         
  if (iIter>=0) SetParams1stIterationMC(pt,h1D,arrMean,arrSigma,MCPars);
  if (iIter>=1) SetParamsMC(islice,pt);
  if (iIter>=2) {   
      
      // Pion kaon and proton amplitude
    if (pt>=1.) {
...
void  SetParams1stIterationMC(Double_t pt, TH1D *h1D, Double_t *arrMean, Double_t *arrSigma, Double_t * MCPars){
  ...
  elMeanMin = TMath::Max(30.,(MCPars[0]-fitWinMean*MCPars[4]));            
  piMeanMin = TMath::Max(30.,(MCPars[1]-fitWinMean*MCPars[5]));            
  kaMeanMin = TMath::Max(30.,(MCPars[2]-fitWinMean*MCPars[6]));                
  prMeanMin = TMath::Max(30.,(MCPars[3]-fitWinMean*MCPars[7]));                
    
        
  elMeanMax = TMath::Min((Double_t)dEdxMax,(MCPars[0]+fitWinMean*MCPars[4]));
  piMeanMax = TMath::Min((Double_t)dEdxMax,(MCPars[1]+fitWinMean*MCPars[5]));
  kaMeanMax = TMath::Min((Double_t)dEdxMax,(MCPars[2]+fitWinMean*MCPars[6]));
  prMeanMax = TMath::Min((Double_t)dEdxMax,(MCPars[3]+fitWinMean*MCPars[7]));
  
        
  elSigmaMin = MCPars[4]/2.;
  piSigmaMin = MCPars[5]/2.;      
  kaSigmaMin = MCPars[6]/2.;
  prSigmaMin = MCPars[7]/2.;
  
    
  elSigmaMax = MCPars[4]*2.;
  piSigmaMax = MCPars[5]*2.;
  kaSigmaMax = MCPars[6]*2.;
  prSigmaMax = MCPars[7]*2.;
...
void FitParticleSampleForKSEstimate(Int_t iks, TH1D *hClean, Double_t *parSample, Double_t *arrMean, Double_t *arrSigma, ParticleType pSpecy, Int_t sampleType){
  ...
  Double_t sampleFitWindow = (sampleType == 0) ? 3.5 : 4.2; 
    ...
    // fit function
  TF1 *asyGaus = new TF1(Form("asyGaus_%d",Int_t(pSpecy)),fitFunctionGenGaus,mean-sampleFitWindow*rms,mean+4.2*rms);   
  ...
  parSample[0] = asyGaus->GetParameter(0);  
...
// -------------------------------------------------------------------------------------------------------
void ProduceGraphs(TTree *t, TObjArray *arr, TObjArray *barr, TGraphErrors **grTmp, TGraph **grSmoothTmp){
  //
...
TGraphErrors * ApplyScalingWrtExpected(TString exp, TString fit, const Double_t safeP, TObjArray * arr, TTree *t){
  ...
  // calculate scaling factor  exmean/fitmean 
