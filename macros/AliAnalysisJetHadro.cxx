...
void AliAnalysisJetHadro::UserCreateOutputObjects()
{
  ...
  if(fEffMatrix && !fRunOnGrid)
  {
    ...
    std::vector<std::vector<Double_t>> effMatrixBins = {particlesBins, centBins, momBins, etaBins, phiBins};
    ...
    Int_t nbinsScan[ndimScan] ={2, 2, fNSettings, 3,
      (Int_t) fEffMatrixCentBins.size() - 1,
      (Int_t) fEffMatrixMomBins.size() - 1,
      (Int_t) fEffMatrixEtaBins.size() - 1
    };
    ...
    std::vector<std::vector<Double_t>> effMatrixScanBins = {detectorBins, originBins, settingsBins, particlesBins, centBins, momBins, etaBins};
  ...
  PostData(7, fTreeMCFull);
  PostData(8, fTreeMCgen);
  PostData(9, fTreedEdxCheck);
  PostData(10, fTreeCuts);
  PostData(11, fTreeDnchDeta);
  PostData(12, fTreeMCFullAcc);
  PostData(13, fTreejetsFJconst);
  PostData(14, fTreeResonance);
  PostData(15, fTreeMCgenMoms);
  PostData(16, fTreeEvents);
  PostData(17, fTreeDScaled);
  PostData(18, fTreejetsFJGen);
  PostData(19, fTreejetEMC);
  PostData(20, fTreeMCEffCorr);
  PostData(21, fTreeExpecteds);
...
Bool_t AliAnalysisJetHadro::Run()
{
  ...
  // ======================================================================
  //
  if (fDEdxCheck)                          { FillTPCdEdxCheck(); return kFALSE;}
  if (fRunFastSimulation && fFillDnchDeta) { FillDnchDeta(); return kFALSE;}
  if (fRunFastHighMomentCal)               { FastGenHigherMoments(); return kFALSE;}
  if (fRunFastSimulation)                  { FastGen_NetParticles(); return kFALSE;}
  //
  // Real Data Analysis
  //
  ...
  if (fMCtrue && fFillHigherMomentsMCclosure){
    ...
    if (fUseCouts)  std::cout << " Info::siweyhmi: (MCclosure Higher Moments) End of Filling part = " << fEventCountInFile << std::endl;
    return kTRUE;
  }
  return kTRUE;
...
void AliAnalysisJetHadro::FindJetsEMC()
{
  ...
  for(auto jet : fJetContainer->accepted())
  {
    ...
    for(Int_t i = 0; i < jet->GetNumberOfParticleConstituents(); i++)
    {
      ...
      Bool_t fBit96_base   = fESDtrackCuts_Bit96->AcceptTrack(esdtrack);
      Bool_t fBit128       = fESDtrackCuts_Bit128->AcceptTrack(esdtrack);
      Bool_t fBit768       = fESDtrackCuts_Bit768->AcceptTrack(esdtrack);
      Bool_t ifDCAcutIfNoITSPixel = ApplyDCAcutIfNoITSPixel(esdtrack);
...
void AliAnalysisJetHadro::FillTPCdEdxReal()
{
  ...
  for (Int_t itrack=0;itrack<event->GetNumberOfTracks();++itrack) {   // Track loop
    ...
    //
	  if (fFillTracks && fFillIncTracks && !fFillOnlyHists)
    {
      if(!fTreeSRedirector) return;
...
//________________________________________________________________________
void AliAnalysisJetHadro::FillMCFull_piKpr()
{
  ...
  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the FillMCFull_piKpr ===== " << std::endl;
  //
  // ======================================================================
...
//________________________________________________________________________
void AliAnalysisJetHadro::FillMCFull_NetParticles()
{
  ...
  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the FillMCFull_NetParticles ===== " << std::endl;
  //
...
//________________________________________________________________________
void AliAnalysisJetHadro::FastGen_NetParticles()
{
  //
  // Fill dEdx information for the TPC and also the clean kaon and protons
  //
  ...
  Int_t nSubSample = 20;
  sampleNo = Int_t(fEventGID)%nSubSample;
  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the FastGen_NetParticles ===== " << std::endl;
  //
  // ======================================================================
  // --------------   MC information with ideal PID   ---------------------
  // ======================================================================
...
//________________________________________________________________________
void AliAnalysisJetHadro::FillGenDistributions()
{
...
//________________________________________________________________________
void AliAnalysisJetHadro::FastGen()
{

  //
  // Fill dEdx information for the TPC and also the clean kaon and protons
  //
  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the FastGen ===== " << std::endl;
  Int_t sampleNo = 0;
  Int_t nSubSample = 20;
  sampleNo = Int_t(fEventGID)%nSubSample;  // ======================================================================
  // ======================================================================
...
//________________________________________________________________________
void AliAnalysisJetHadro::FastGenHigherMoments()
{

  //
  // Fill dEdx information for the TPC and also the clean kaon and protons
  //
  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the FastGenHigherMoments ===== " << std::endl;
...
//________________________________________________________________________
void AliAnalysisJetHadro::MCclosureHigherMoments()
{
  ...
  Int_t evtNuminFile = fMCEvent -> GetEventNumberInFile();
  if (fUseCouts) {
    std::cout << " Info::siweyhmi: ===== In the MCclosureHigherMoments ===== sampleNo = " << sampleNo;
...
void AliAnalysisJetHadro::FillEffMatrix()
{
  ...
  for (Int_t iset=0; iset<fNSettings; iset++) {
    ...
    if ( iset==kCutReference || iset==kCutEventVertexZLarge ) {
      //
      for (Int_t iTrack = 0; iTrack < fMCEvent->GetNumberOfTracks(); iTrack++)
      { // track loop
        //
...
//________________________________________________________________________
void AliAnalysisJetHadro::WeakAndMaterial()
{
  //
  // Fill dEdx information for the TPC and also the clean kaon and protons
  //
  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the WeakAndMaterial ===== " << std::endl;
  //
  // ======================================================================
  // ======================================================================
  //
...
//________________________________________________________________________
void AliAnalysisJetHadro::FillDnchDeta()
{

  //
  // Fill dEdx information for the TPC and also the clean kaon and protons
  //
  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the FillDnchDeta ===== " << std::endl;
  //
  // ======================================================================
  // ======================================================================
  //
...
//________________________________________________________________________
void AliAnalysisJetHadro::FillTPCdEdxCheck()
{
  //
  // Fill dEdx information for the TPC and also the clean kaon and protons
  //
  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the FillTPCdEdxCheck ===== " << std::endl;
  AliVEvent *event=InputEvent();
  //
  // Main track loop
  //
...
//________________________________________________________________________
void AliAnalysisJetHadro::CreateEventInfoTree()
{

  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the CreateEventInfoTree ===== " << std::endl;
...
//________________________________________________________________________
void AliAnalysisJetHadro::CalculateEventInfo()
{

  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the CalculateEventInfo ===== " << std::endl;
...
//________________________________________________________________________
Int_t AliAnalysisJetHadro::CacheTPCEventInformation()
{
  ...
  if (fUseCouts) printf(" Info::siweyhmi: ===== In the CacheTPCEventInformation:  %d\n",selected);
  return selected;
}
//________________________________________________________________________
void AliAnalysisJetHadro::DumpDownScaledTree()
{

  if (fUseCouts) std::cout << " Info::siweyhmi: ===== In the DumpDownScaledTree ===== " << std::endl;
...
Bool_t AliAnalysisJetHadro::CheckPsiPair(const AliESDv0* v0)
{
